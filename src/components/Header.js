...
import Members from '../containers/MemberContainer.js'
    ...
    render(){
        ...
        <div id='header'>
            <div id='header-text'>
                ...
                <p>Members: {this.props.playlistUsers.length > 0 ? this.props.playlistUsers.map(user => <a href={user.spotify_uri}>{user.name}</a>) : 'No Members Yet'}</p>
            </div>
            ...
            <>
            <div id='download-button'><Button color='teal' onClick={this.handleDownload}>Download/Update to Spotify<Icon name='arrow alternate circle down outline'></Icon></Button></div>
          <Form id='user-form'>
            <Input onChange={this.handleChange} value={this.state.uri} placeholder='Add Spotify User To Group'></Input><br></br><Label basic color='blue' pointing >Enter Spotify URI</Label><Button onClick={this.handleAddUser}>Submit</Button></Form></> : null }
         </div>
        )
    }
}
